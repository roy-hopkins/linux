...
static int snp_launch_update_vmsa(struct kvm *kvm, struct kvm_sev_cmd *argp)
{
	...
	kvm_for_each_vcpu(i, vcpu, kvm) {
		 ...
		 */
		if (!has_snp_feature(sev, SVM_SEV_FEAT_SVSM) || !i) {
...
static int __sev_snp_update_protected_guest_state(struct kvm_vcpu *vcpu)
{
	...
	if (VALID_PAGE(svm->sev_es.snp_vmsa[svm->sev_es.snp_target_vmpl].gpa)) {
		 ...
		 */
		svm->sev_es.snp_has_guest_vmsa[svm->sev_es.snp_target_vmpl] = true;

		/* Use the new VMSA */
		svm->sev_es.vmsa_pa[svm->sev_es.snp_target_vmpl] = pfn_to_hpa(pfn);
		svm->vmcb->control.vmsa_pa = svm->sev_es.vmsa_pa[svm->sev_es.snp_target_vmpl];
...
static int sev_snp_ap_creation(struct vcpu_svm *svm)
{
	...
	case SVM_VMGEXIT_AP_CREATE:
		...
		target_svm->sev_es.snp_vmsa[vmpl].gpa = svm->vmcb->control.exit_info_2;
		break;
...
static bool snp_cleanup_guest_buf(struct kvm *kvm, struct sev_data_snp_guest_request *data)
{
				   ...
				   sev_ret_code *fw_err)
{
	...
	if (!snp_cleanup_guest_buf(kvm, &data))
		ret = false;
...
static int snp_begin_ext_guest_req(struct kvm_vcpu *vcpu)
{
	...
	vcpu->run->exit_reason = SVM_EXIT_VMGEXIT;
...
static void sev_es_init_vmcb(struct vcpu_svm *svm)
{
	 ...
	 */
	if (svm->sev_es.vmsa_pa[svm->sev_es.snp_current_vmpl] && 
	    !svm->sev_es.snp_has_guest_vmsa[svm->sev_es.snp_current_vmpl]) {
...
void sev_es_vcpu_reset(struct vcpu_svm *svm)
{
	...
	for (i = 1; i < SVM_SEV_VMPL_MAX; i++) {
		...
		svm->sev_es.snp_has_guest_vmsa[i] = false;
		svm->sev_es.snp_vmsa[i].ap_create = false;
	}
}
